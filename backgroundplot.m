...
function P_p = backgroundplot(FILE,bn,n)
...
% hold on;plot(Y_BG,Y_val,'ok');title('YFP');
 ...
 R_p_v = polyval(R_p,Y_BG./C_BG);
 Y_p_v = polyval(Y_p,Y_BG); 
 C_p_v = polyval(C_p,C_BG);  
 P_p = [Y_p; C_p; R_p];
 
 FILENAME3 = sprintf('%s/YFP_back.png',FILE);
 figure;plot(Y_BG,Y_val,'o','LineWidth',5);title('YFP');
 hold on; plot(Y_BG,Y_p_v,'-');
 print('-dpng',FILENAME3);
 figure;plot(C_BG,C_val,'o','LineWidth',5);title('CFP');
 hold on; plot(C_BG,C_p_v,'-'); 
 FILENAME4 = sprintf('%s/CFP_back.png',FILE);
 print('-dpng',FILENAME4);
 figure;plot(Y_BG./C_BG,Y_val./C_val,'o','LineWidth',5); ...
     title('RATIO');
 hold on; plot(Y_BG./C_BG,R_p_v,'-');  
 FILENAME5 = sprintf('%s/RATIO_back.png',FILE);
