...
% with correcting time by image information
%
% ima -> for checking with eyes

function IM_RATIO = im_split_ontime(FILENAME,N,calY,calX)
%making background image
BackGround = average_image(FILENAME); %  background
...
TAG_B = imfinfo(FILE);
IM_Date_B = TAG_B.DateTime;
T_ref_B = Time_reference(IM_Date_B);
...
%  FILE = sprintf('%s%1.4d.TIF',FILENAME,i);  
  ...
  IM = imread(FILE);
  TAG = imfinfo(FILE);
  IM_Date = TAG.DateTime;
  T_ref = Time_reference(IM_Date);
  Time_Point = T_ref - T_ref_B;
...
%  IM_CFP = IM(200:end,WIDTH+1:end);   %%%%%%%%%%%%chenge!!
  ...
  BackGroundY = BackGround(:,1:WIDTH); %  background
  BackGroundC = BackGround(:,WIDTH+1:end); %  background
...
%figure;imagesc(IM_YFP);
%figure;imagesc(IM_CFP);
...
%                              Neuron_Xcoordinate-2:Neuron_Xcoordinate+2))); ...
  ...
  Back_Y = double(BackGroundY(YFP_max_y,YFP_max_x));   % background
  Back_C = double(BackGroundC(YFP_max_y-yoffset,YFP_max_x-xoffset));  % background
  YFP_VAL = double(IM_YFP(YFP_max_y, YFP_max_x));
  CFP_VAL = double(IM_CFP(YFP_max_y-yoffset,YFP_max_x-xoffset));
%  Back_Y = mean(mean(BackGroundY(YFP_max_y-1:YFP_max_y+1, YFP_max_x-1:YFP_max_x+1)));%  background
%  Back_C = mean(mean(BackGroundC(YFP_max_y-1-yoffset:YFP_max_y+1-yoffset, YFP_max_x-1-xoffset:YFP_max_x+1-xoffset)));%  background  
...
%  IM_RATIO(i).data = RATIO;
  ...
  IM_RATIO(i).BGY = Back_Y;
  IM_RATIO(i).BGC = Back_C;  
  IM_RATIO(i).Time_Point = Time_Point;
end
