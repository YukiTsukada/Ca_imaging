...
function IM_RATIO = static_ratio(FILENAME,Num,ST,N)
FILE = sprintf('%s%1.4d.TIF',FILENAME,ST);
...
for k = 1:Num
  Neuron_Ycoordinate = Position_Y(k) + Margin;
  Neuron_Xcoordinate = Position_X(k) + Margin;
  ...
  RATIO_VAL = mean(mean(RATIO(Neuron_Ycoordinate-2:Neuron_Ycoordinate+2, ...
      ...
      if(k==1)
        ...
        IM_RATIO(i).value = [IM_RATIO(i).value RATIO_VAL];
      end 
